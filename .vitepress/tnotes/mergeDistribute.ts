 ...
 * 用于批量编辑所有笔记内容
 */
export function mergeNotes(): void {
  /**
   * dirMap 映射表
   * - key: readme 的绝对路径
  ...
  for (const key in dirMap) {
    try {
      ...
      console.error(`Failed to read or process ${key}:`, errorMessage)
    }
  }
...
export function distributeNotes(): void {
  ...
  mergedReadmeSections
    ...
    .forEach((section) => {
      ...
      if (!linkMatch) {
        console.error(`Invalid section: ${lines[titleLineIndex]}`)
        return
      }
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : String(error)
        console.error(`"❌ ${notesPath}" failed to write:`, errorMessage)
      }
    })
